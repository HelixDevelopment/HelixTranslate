...
// (W3), but via the metadata fields rather than the text. It is realistic
// because some model identifiers natively contain ':'
...
func TestW20_MakeCacheKey_NoDelimiterInjectionCollision(t *testing.T) {
	...
	keyA := r.makeCacheKey("Дом", "ru", "en", "local", "llama3:8b")
	keyB := r.makeCacheKey("Дом", "ru", "en", "local:llama3", "8b")
	...
	if keyA == keyB {
		t.Fatalf("delimiter-injection collision: distinct (provider,model) tuples share a cache key\n"+
			...
			"local", "llama3:8b", "local:llama3", "8b", keyA)
	}

	// A second realistic instance: a model whose colon collides with a sibling.
	keyC := r.makeCacheKey("Дом", "ru", "en", "local", "qwen2.5:7b")
	keyD := r.makeCacheKey("Дом", "ru", "en", "local", "qwen2.5:7b:extra")
	keyE := r.makeCacheKey("Дом:extra", "ru", "en", "local", "qwen2.5:7b") // injection via text? text is hashed, so safe — keep as control
	if keyC == keyD {
