...
- **DOCX output** is built and proven by an earlier produced file (a real `.docx` = "Microsoft Word 2007+" with real translated text). **PDF output is built** and proven by passing tests (it renders the translated book to a real, valid PDF that keeps Serbian Cyrillic readable). **⚠️ Screen recordings of both are now BLOCKED, not merely owed:** while recording Wave 1 we discovered the default command-line/gRPC translation path is currently runtime-broken (the auto-selected verified model is rejected by a stale model whitelist), so a fresh DOCX/PDF run through the tool does not complete. We did NOT fake a "working" recording. This is a HIGH-priority fix (see the engineering page, gap 1).
...
- **Video proof is still the main gap:** **25 of 493** features have a recorded, watch-it-yourself demonstration (≈ 5.1%). Everything else is built and present in the code, but a recording is still owed. Nothing is claimed proven just because the code exists.
...
**25 / 493 ≈ 5.1%**, plus **2 real-artifact/test-confirmed** (DOCX + PDF output, video PENDING). This is the headline number to keep improving: each new genuine recording raises it.
...
| Video-confirmed | 25 (+ 2 real-artifact/test-confirmed: DOCX + PDF output) |
...
1. **Video-confirmation 25/493** (+ 2 real-artifact/test-confirmed: DOCX + PDF output). 25 rows carry real, ffprobe- and content-verified recordings. **Wave 1 (2026-06-16) added 2:** `helix_translate-bridge-ensemble-translate-20260616-104846.mp4` (best-model selection + provider-diverse fallback + real `invoke`→"Buenos días, querido amigo."; ffprobe 7.77s/33fr/8 distinct frame-hashes) and `helix_translate-bridge-verified-catalogue-translate-20260616-105327.mp4` (`model-bridge list` provider-diverse catalogue + real `invoke`→"Le pont relie deux rives."; ffprobe 13.71s/40fr/6 distinct). Prior recordings: the `-multipass` video `helixtranslate-cli-multipass-verify-20260615.mp4` (888x630/93fr/9.3s); the Wave-0 bridge CLI video `helixtranslate-bridge-bestmodel-translate-20260615.mp4` (790×560/80fr/8.0s); the bridge MCP-stdio video `helixtranslate-bridge-mcp-stdio-20260615.mp4` (790×560/10fr/11.88s); the 3 video-surfaced bug fixes in commit `a5e8866`. Per §11.4.2/§11.4.107 nothing may be marked video-confirmed without a real, content-verified file.
   - **🔴 Wave-1 DISCOVERED BLOCKER (§11.4.108/§11.4.138):** the default bridge **translator** path (`unified-translator` + gRPC) is RUNTIME-BROKEN — `VerifiedFactory.CreateTranslatorWithFallback`→`NewLLMTranslatorWithConfig` rejects the verified strongest model `novita/Sao10K/L3-8B-Stheno-v3.2` against the stale `ValidModels` whitelist (`llm.go:237`), while the `BestModel`/`invoke` path (the recorded videos) correctly bypasses it. This BLOCKS Wave-1 videos #2 DOCX / #3 PDF / #4 gRPC / per-provider-unified (honest SKIPs, not faked PASS). HIGH severity — release-critical default path. Fix is a dedicated Wave-2 item (route VerifiedFactory through the ResolvedProvider resolver, bypassing the whitelist for verified models). See Status.md "Honest gaps" caveat 8.
...
`25 / 493` enumerated rows = **5.1%** (≈ 6.0% against the 417 headline). Plus 2 real-artifact/test-confirmed (DOCX + PDF output, video PENDING). This is the primary metric to drive upward; each anti-bluff §11.4.107 recording increments the numerator.
