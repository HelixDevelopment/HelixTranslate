...
| Video-confirmed | **22** (real, content-verified recordings this session — see Anti-bluff note) — ≈ 4.6% of 478 enumerated rows (≈ 5.3% of the 417 headline). Plus **2 real-artifact/unit-confirmed** (DOCX output — produced `.docx` = Microsoft Word 2007+; PDF output — `pkg/ebook` tests green + weasyprint present; both video PENDING). Rev-5 (this run) flips the **LLMsVerifier→agent bridge CLI** row video PENDING→CONFIRMED (`helixtranslate-bridge-bestmodel-translate-20260615.mp4` — live `novita/Sao10K/L3-8B-Stheno-v3.2` EN→ES translation, ffprobe-verified). The MCP-stdio row stays video PENDING (the recording shows best-model+invoke, not the MCP surface — no bluff). Other Rev-4 additions (pkg/bridge, internal/verifier gate, pkg/api/dashboard.go) are PASS at unit/integration/live-run level with video PENDING / SKIP / N/A — none claim a video that doesn't exist |
