...
| bridge | `EnsembleFactory` (provider-diverse ensemble seam) | Library | Implemented | Wired (R-1 ensemble redirect) | `ensemble.go:25` — context-only closure yielding the provider-diverse verified translator set (one per distinct provider, strongest-first); structural shape of the per-component `EnsembleTranslatorFactory` seams, passed to `NewMultiLLMCoordinatorWithFactory` / `NewPreparationCoordinatorWithFactory` / `NewBookPolisherWithFactory` WITHOUT those packages importing the bridge (§11.4.28); no API key captured/logged (§11.4.10) | Unit-covered (`pkg/bridge/ensemble_test.go`, -race green) — masks the runtime bug | **PARTIAL — UNIT PASS, RUNTIME BLOCKED (§11.4.108):** backed by `ProviderDiverseTranslators`→`NewLLMTranslatorWithConfig`; same `ValidModels`-whitelist runtime failure (see Wave-1 discovered blocker) | PENDING — NOT recordable until the blocker is fixed |
| bridge | `ProviderDiverseTranslators` | Library | Implemented | Wired (backs `EnsembleFactory`) | `bridge.go:527` — returns provider-diverse verified `translator.Translator` set (one per distinct provider, strongest-first); the ensemble translator source | Unit-covered (`pkg/bridge/bridge_test.go`, `pkg/bridge/ensemble_test.go`, -race green) — masks the runtime bug | **PARTIAL — UNIT PASS, RUNTIME BLOCKED (§11.4.108):** builds each translator via `NewLLMTranslatorWithConfig`, which rejects verified non-whitelisted models (see Wave-1 discovered blocker) | PENDING — NOT recordable until the blocker is fixed |
...
1. **Video coverage is 25 feature rows** (+ 2 real-artifact/unit-confirmed). 25 features are confirmed by real, ffprobe- and content-verified recordings (Wave 1 of 2026-06-16 added 2 — bridge-ensemble best-model selection + bridge provider-diverse catalogue; see the Anti-bluff note + the Coverage-summary `Video-confirmed` row for the authoritative count and full list — reconciled with both, §11.4.60); DOCX output and PDF output are real-artifact/unit-confirmed (DOCX = produced `.docx` = Microsoft Word 2007+; PDF = `pkg/ebook` tests green + weasyprint present) with their videos still PENDING and now BLOCKED (see caveat 8 — the bridge-translator-path runtime blocker). Every other runtime/user-visible feature is `PENDING` a recording or `N/A`. This is stated, not hidden.
