...
| Enumerated feature rows in this doc | **493** (Rev 8 — RE-DERIVED MECHANICALLY from the live tables, §11.4.6: count of 9-column feature data rows across the 60 per-category tables; `awk` counting rows with exactly 9 columns, excluding the 60 header rows + separators. The earlier "478" was NOT re-derivable from the live table — it was a hand-tracked running tally that had drifted (the actual live count was 486 before Rev 8 added the 7 missing `pkg/bridge` ensemble-seam rows: `BestTranslator`, `BestTranslatorFunc`, `EnsembleFactory`, `ProviderDiverseTranslators`, `ProviderDiverseClients`, `ProviderDiverseModels`, `BestClient`). 493 is the FACT; re-derive with `awk` over `^\|` 9-column rows) |
| Implemented | 442 (Rev 8: 435 Rev 7 + 7 new pkg/bridge ensemble-seam rows; was 471 Rev 6, −36 flipped to Obsolete Rev 7 — local-runtime + SSH-local features removed in bridge phase-2 R-2..R-4) |
...
1. **Video coverage is 23 feature rows** (+ 2 real-artifact/unit-confirmed). 23 features are confirmed by real, ffprobe- and content-verified recordings this session (see the Anti-bluff note + the Coverage-summary `Video-confirmed` row for the authoritative count and full list — this number is reconciled with both, §11.4.60); DOCX output and PDF output are real-artifact/unit-confirmed (DOCX = produced `.docx` = Microsoft Word 2007+; PDF = `pkg/ebook` tests green + weasyprint present) with their videos still PENDING. Every other runtime/user-visible feature is `PENDING` a recording or `N/A`. This is stated, not hidden.
